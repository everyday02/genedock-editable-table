...
class AntEditableTable extends Component {
  ...
  transformColumns() {
    ...
    formatColumns.push({
      ...
      operation: true,
      width: 200,
      render: (text, record, index) => this.operationRender(this.state.data, text, record, index)
    });
  ...
  wrapperColumnsRender(columns) {
    columns.map((item) => {
      if (!item.render) {
        item.render = (text, record, index) => this.renderColumns(this.state.data, index, item.id, text, item);
      }
      return item;
    });
    return columns;
